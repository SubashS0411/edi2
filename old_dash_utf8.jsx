...
// Dynamic admin email ΓÇö read from .env so it stays in sync when credentials are changed
...
const AdminDashboard = () => {
    ...
    useEffect(() => {
        ...
        // Admin guard ΓÇö priority order:
        //  1. profiles table role  (authoritative ΓÇö works for manually-created accounts)
    ...
    const onRemoveUser = async (req) => {
        ...
        if (result.success) {
            // Immediately remove from local state ΓÇö no re-fetch needed
        ...
        <div className="min-h-screen bg-[#050510] text-white p-6 sm:p-10 font-sans selection:bg-purple-500/30">
            ...
            <div className="max-w-[1400px] mx-auto space-y-12 relative z-10">
                ...
                >
                    ...
                    {/* Live Admin Credentials Panel ΓÇö reads from app_settings DB */}
                    <div className="mx-6 mt-2 mb-1 p-4 rounded-2xl bg-indigo-500/5 border border-indigo-500/20">
                        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                            ...
                            {/* Live Password ΓÇö masked by default, reveal on click */}
                            <div className="flex items-center gap-1.5 bg-white/5 border border-white/10 px-3 py-1.5 rounded-lg font-mono text-xs text-slate-300">
                                <KeyRound className="w-3 h-3 text-amber-400 flex-shrink-0" />
                                <span className="tracking-widest select-none">
                                    {currentAdminPassword
                                        ? (showCurrentPassword ? currentAdminPassword : 'ΓÇó'.repeat(currentAdminPassword.length))
                            ...
                            <p className="text-[11px] text-slate-500 sm:ml-auto">
                                Live from database ΓÇö no <code className="bg-white/10 px-1 rounded">.env</code> changes needed after updates.
                            </p>
                        </div>
                        ...
                        {!currentAdminPassword && (
                            <p className="mt-2 text-[11px] text-amber-500/80 flex items-center gap-1.5">
                                ...
                                Password not yet stored in database. Enter a new password below ΓÇö it will be saved dynamically.
                            </p>
                        )}
                    </div>
                                ...
                                <div className="p-6">
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        <div className="bg-white/5 rounded-xl p-3 border border-white/5">
                                            ...
                                            <p className="text-sm font-medium text-white truncate">{dbProfile.profile?.full_name || 'ΓÇö'}</p>
                                        </div>
                                        <div className="bg-white/5 rounded-xl p-3 border border-white/5">
                                            <p className="text-xs text-slate-500 mb-1 uppercase tracking-wider">Email (Auth)</p>
                                            <p className="text-sm font-medium text-white truncate">{dbProfile.authEmail || 'ΓÇö'}</p>
                                        </div>
                                        <div className="bg-white/5 rounded-xl p-3 border border-white/5">
                                            <p className="text-xs text-slate-500 mb-1 uppercase tracking-wider">Email (Profiles)</p>
                                            <p className="text-sm font-medium text-white truncate">{dbProfile.profile?.email || 'ΓÇö'}</p>
                                        </div>
                                        <div className="bg-white/5 rounded-xl p-3 border border-white/5">
                                            <p className="text-xs text-slate-500 mb-1 uppercase tracking-wider">Role</p>
                                            <p className="text-sm font-medium text-emerald-400 capitalize">{dbProfile.profile?.role || 'ΓÇö'}</p>
                                        </div>
                                    </div>
                                        ...
                                        >
                                            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                                                ...
                                                <div className="flex items-center gap-2 mt-4 md:mt-0 flex-wrap">
                                                    {/* Admin accounts are protected ΓÇö show badge instead of action buttons */}
                                                    {isAdminUser(req) ? (
                                                        ...
                                                        <>
                                                            ...
                                                            {/* Remove User ΓÇö visible for non-admin users only */}
                                                            <Button
